    ...
    var api = {
        ...
        post:function(fileInfo, file, finish){
			...
			fs.readFile(file.path, function(err, fileBuffer){
				if (err) return finsih(err);
				uploadFile(s3, fileInfo.name, fileBuffer, opts, function(err, awsFile){
					...
					var count = Object.keys(opts.imageVersions).length;
					thumbnail(fileInfo, fileBuffer, opts.imageVersions, function(version, buffer){
						uploadFile(s3, path.join(version, fileInfo.name), buffer, opts, function(err, awsFile){
							if (0 >= --count){
								fileInfo.proccessed = true;
								fileInfo.initUrls();
								finish(err, fileInfo);
        ...
        delete:function(req,res,callback){
            var params = {
                ...
                Key: path.join(api.options.uploadDir, path.basename(req.url))
            };
			var s3 = this.s3;
            s3.deleteObject(params, function(err, data) {
                if (err) return callback(err);

				Object.keys(api.options.imageVersions).forEach(function(version) {
					params = {
						Bucket: configs.bucketName,
						Key: path.join(api.options.uploadDir, version, path.basename(req.url))
					};
...
function uploadFile(s3, fileName, fileBuffer, config, callback) {
