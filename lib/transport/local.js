...
module.exports = function(opts, storage) {
    var api = {
        ...
        delete: function(req, res, callback) {
            var fileName = '';
            if (req.url.slice(0, opts.uploadUrl.length) === opts.uploadUrl) {
                fileName = path.basename(decodeURIComponent(req.url));
                if (fileName[0] !== '.') {
                    fs.unlink(path.join(storage.uploadDir, fileName), function(ex) {
						if (storage.imageVersions){
							Object.keys(storage.imageVersions).forEach(function(version) {
								// TODO - Missing callback
								fs.unlink(path.join(storage.uploadDir, version, fileName), function(){});
							});
						}
                        callback(null, {
                            success: true
